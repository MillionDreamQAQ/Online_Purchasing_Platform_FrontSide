...
import { getAllQuotations } from '@/request/quotationRequest';
import { IQuotation } from '@/request/model';
...
const templateData: TemplateDataType[] = [
    ...
    },
    {
        ...
        size: '2000*2000*250',
        unit: '件',
        desc: '金属框架E1级中纤维板贴木皮工艺优质环保油漆喷涂饰面优质五金件连接'
    },
    {
        key: '3',
        name: '床头柜',
...
export class QuotationTableDeprecated extends React.Component {
    private selectedRows = [] as TemplateDataType[];

    private designer: GCD.Spread.Sheets.Designer.Designer | undefined;

    private quotations: IQuotation[] = [];

    async componentDidMount() {
        const res = await getAllQuotations();
        const quotations = res.data.map(({ quotationName, template }) => ({
            quotationName,
            template
        }));
    ...
    state = {
        addQuotationDrawerVisible: false,
        templateSetDrawerVisible: false,
        contentShownIndex: 1,
    ...
    private closeAddQuotationDrawer = () => {
        this.setState({
            addQuotationDrawerVisible: false
        });
    };

    private openAddQuotationDrawer = record => {
        this.setState({
            addQuotationDrawerVisible: true,
            selectedRowsIndex: record.key - 1
        });
    };
    ...
    private getAddQuotationName = e => {
        this.setState({
            addQuotationName: e.target.value
        });
    };

    private clickAddQuotationButtonHandler = () => {
        if (this.state.addQuotationName === '') {
            message.error({
                content: '报价单名称不能为空',
                duration: 1,
                style: {
            ...
            quotationTableData.push({
                key: (quotationTableData.length + 1).toString(),
                name: this.state.addQuotationName
            });
        }
    };
        ...
        return (
            <div>
                <Button
                    ...
                    onClick={this.openAddQuotationDrawer}
                    type='primary'
                >
                    添加报价单
                </Button>
                ...
                <Table
                    rowKey={quotationTableDat => quotationTableDat.key.toString()}
                    columns={this.quotationTableColumns}
                ...
                <Drawer
                    ...
                    open={this.state.templateSetDrawerVisible}
                >
                    <div>
                        <Table
                            rowSelection={{
                ...
                <Drawer
                    ...
                    size='default'
                    placement='right'
                    onClose={this.closeAddQuotationDrawer}
                    open={this.state.addQuotationDrawerVisible}
                >
                    <div>
                        <Input
                            className={scssStyles.quotationNameInput}
                            onChange={this.getAddQuotationName}
                            placeholder='输入报价单名称'
                        />
                        <Button type='primary' onClick={this.clickAddQuotationButtonHandler}>
                            添加报价单
                        </Button>
                    </div>
                </Drawer>
